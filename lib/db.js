 ...
 *
 * Usage (serverless API routes - api/detections.js, api/stats.js, api/alerts.js,
 * api/telegram/webhook.js):
 *   const { getDb } = require('../lib/db');
 *   const db = await getDb();
 *   await db.collection('detections').find(...).toArray();
 */
...
/**
 * Cached raw Db handle for serverless routes that need direct
 * db.collection(...) access (aggregate, distinct, countDocuments, custom
 * queries) rather than the saveDetection/getDetections store interface above.
 *
 * Reuses a single MongoClient across warm invocations of the same
 * serverless